...
class Box:
    ...
    def draw(self):
        ...
        if self.selected:
            ...
            #arcade.draw_texture_rectangle(self.x, self.y-self.BOX_SIZE/2, 150,96,'Forklift.png') 
...
class LogisticsTetris(arcade.View):
    ...
    def on_update(self, delta_time):
        ...
        for box in self.otherboxes:
            ...
            if len(self.boxes)>1 and abs(self.boxes[-1].x - self.boxes[-2].x) < min(self.boxes[-1].BOX_SIZE,self.boxes[-2].BOX_SIZE) and abs(self.boxes[-1].y - self.boxes[-2].y) < min(self.boxes[-1].BOX_SIZE,self.boxes[-2].BOX_SIZE):
...
class InstructionScreen(arcade.View):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.start_text = ['Try to keep the boxes spawned within the warehouse and deliver them as required.',
            'Every box delivered gives you a time boost.','Once you run out of time or let the blocks stack up at the spawn location, You lose',
    ...
    def on_draw(self):
        ...
        arcade.draw_text(self.start_text[3], SCREEN_WIDTH//2, SCREEN_HEIGHT//2 - 50, arcade.color.BLACK, font_size = 20, anchor_x="center", anchor_y="center")
