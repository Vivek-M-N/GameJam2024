...
class Circle:
    ...
    def update(self, delta_time):
        ...
        if self.vx == 0:
            ...
            self.vx_new = [(-b**2 + math.sqrt(1+4*alpha*c))/(-2*alpha)]
            self.vx_new.append((1 - math.sqrt(1+4*alpha*c))/(-2*alpha))
        ...
        if self.vy == 0:
            ...
            self.vy_new = [(-b**2 + math.sqrt(b**2 + 4*alpha*c))/(-2*alpha)]
            self.vy_new.append((1 - math.sqrt(b**2 + 4*alpha*c))/(-2*alpha))
